...
def load_sales(uploaded_file: BytesIO) -> pd.DataFrame:
    ...
    if mapping is None:
        ...
        item_default = st.session_state["sales_mapping"].get("item") or hints.get("item") or (options[0] if options else "")
        qty_default = st.session_state["sales_mapping"].get("qty") or hints.get("qty") or (options[0] if options else "")
