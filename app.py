...
def extract_report_dates(uploaded_file):
    """Parse dates from a title like
    'Product Sales Report from Aug 03, 2025 at 12:00 AM AST to Aug 09, 2025 at 09:06 PM AST'.
...
".join(text.splitlines()[:50])
        low = head.lower()
        i = low.find(" from ")
        j = low.find(" to ", i + 1) if i != -1 else -1
        if i != -1 and j != -1:
            left = head[i + 6:j]
            right = head[j + 4: j + 80].split("
")[0]
            left = left.split(" at ")[0].strip()
            right = right.split(" at ")[0].strip()
            d1 = pd.to_datetime(left, errors="coerce")
            d2 = pd.to_datetime(right, errors="coerce")
            s = d1.date() if pd.notna(d1) else None
            e = d2.date() if pd.notna(d2) else None
            return s, e
    except Exception:
        pass
    return None, None
...
def extract_report_dates_safe(uploaded_file):
    """Safer duplicate of extract_report_dates that fixes newline parsing.
    Returns (start_date, end_date) or (None, None).
    """
    try:
        data = uploaded_file.getvalue() if hasattr(uploaded_file, "getvalue") else uploaded_file.read()
...
def _suggest_map_import(df: pd.DataFrame):
    """Guess columns for mapping import (item, product, buns_per_unit)."""
    cols = list(df.columns)
    item_syn = {
        "item", "item name", "menu item", "modifier name", "product name", "plu name", "description",
        "nombre", "articulo", "artículo", "producto"
    }
    product_syn = {
        "product", "bun kind", "bun", "bun type", "bread", "bread type", "producto", "product name"
    }
    bpu_syn = {
        "buns per unit", "buns_per_unit", "buns", "bun_per", "buns_per", "units per", "units_per",
        "qty per", "qty_per", "ratio", "consumption"
    }
    return {
        "item": _guess_col(cols, item_syn),
        "product": _guess_col(cols, product_syn),
        "buns_per_unit": _guess_col(cols, bpu_syn),
    }


def _normalize_product_value(val: str, valid_products: List[str]) -> Optional[str]:
    """Map free-text product values to canonical names in valid_products.
    Handles case-insensitive exact matches and common synonyms.
    """
    if val is None:
        return None
    v = str(val).strip().lower()
    canon = {p.lower(): p for p in valid_products}
    if v in canon:
        return canon[v]
    synonyms = {
        "hamburger": "Hamburger",
...
def load_sales(uploaded_file: BytesIO) -> pd.DataFrame:
    ...
    """
    try:
        # Try to parse date range from report title/header for files without per-row dates
        sugg_start, sugg_end = extract_report_dates_safe(uploaded_file)
    ...
    if mapping is None:
        ...
        # Helpful preview so you know what to select
        with st.expander("Preview your uploaded data (first 20 rows)", expanded=True):
            st.dataframe(df.head(20))
            st.caption("Columns detected: " + ", ".join([str(c) for c in df.columns]))

        # Remember last manual mapping for this session
        if "sales_mapping" not in st.session_state:
            st.session_state["sales_mapping"] = {}

        options = list(df.columns)
        options_with_none = ["<No date column>"] + options
        ...
        hints = _suggest_mapping(df)
        date_default = st.session_state["sales_mapping"].get("date") or hints.get("date") or "<No date column>"
        # Prefer known headers if present
        item_default = (st.session_state["sales_mapping"].get("item")
                         or next((c for c in options if str(c).strip().lower() == "product"), None)
                         or hints.get("item")
                         or (options[0] if options else ""))
        qty_default = (st.session_state["sales_mapping"].get("qty")
                         or next((c for c in options if str(c).strip().lower() == "sold"), None)
                         or hints.get("qty")
                         or (options[0] if options else ""))

        date_choice = st.selectbox("Date column", options_with_none, index=_idx(date_default, options_with_none), key="map_date_col")
        item_col = st.selectbox("Item column", options, index=_idx(item_default, options), key="map_item_col")
        qty_col = st.selectbox("Quantity column", options, index=_idx(qty_default, options), key="map_qty_col")

        # Persist the user's choice so we don't ask again this session
        st.session_state["sales_mapping"] = {"date": (None if date_choice == "<No date column>" else date_choice), "item": item_col, "qty": qty_col}
        mapping = {"date": (None if date_choice == "<No date column>" else date_choice), "item": item_col, "qty": qty_col}

    # If we don't have a date column, let user assign a single date to all rows
    if mapping.get("date") is None or mapping.get("date") not in df.columns:
        st.info("No date column detected. Choose how to assign dates to these totals.")
        mode = st.radio("Date assignment", ["Single date", "Distribute across a range"], horizontal=True, key="no_date_mode")
        if mode == "Single date":
            default_date = sugg_end or datetime.today().date()
            chosen_date = st.date_input("Date for all rows", value=default_date, key="fallback_single_date")
            out = pd.DataFrame({
                "date": [chosen_date] * len(df),
            ...
            c1, c2 = st.columns(2)
            with c1:
                default_start = sugg_start or (datetime.today().date() - timedelta(days=6))
            default_end   = sugg_end   or  datetime.today().date()
            start_date = st.date_input("Start date", value=default_start, key="fallback_range_start")
            with c2:
                end_date   = st.date_input("End date",   value=default_end,   key="fallback_range_end")
            if end_date < start_date:
                st.error("End date must be on or after start date.")
                st.stop()
            dates = pd.date_range(start=start_date, end=end_date, freq="D").date
            dist_method = st.selectbox("Distribution", ["Evenly across days", "Weighted (80% weekdays / 20% weekend)"], key="fallback_dist_method")
            # Build weights
            if dist_method.startswith("Even"):
                weights = np.ones(len(dates), dtype=float)
            else:
                # 80% across Mon-Fri, 20% across Sat-Sun
                wk_days = [d for d in dates if pd.to_datetime(d).weekday() <= 4]
                we_days = [d for d in dates if pd.to_datetime(d).weekday() >= 5]
                wk_w = (0.8 / len(wk_days)) if wk_days else 0.0
                we_w = (0.2 / len(we_days)) if we_days else 0.0
                weights = np.array([wk_w if pd.to_datetime(d).weekday() <= 4 else we_w for d in dates], dtype=float)
            # Normalize to sum=1 (in case of edge cases)
            weights = weights / max(1e-9, weights.sum())

            # Expand rows per-day using integer allocation that preserves totals
            items_list = df[mapping["item"]].astype(str).str.strip().tolist()
            qtys = pd.to_numeric(df[mapping["qty"]], errors="coerce").fillna(0).astype(int).tolist()
            rows = []
            for item_name, total in zip(items_list, qtys):
                raw = np.floor(weights * total).astype(int)
                remainder = int(total - raw.sum())
                if remainder > 0:
                    raw[:remainder] += 1
                for d, q in zip(dates, raw):
                    if q > 0:
                        rows.append({"date": d, "item": item_name, "qty": int(q)})
            out = pd.DataFrame(rows) if rows else pd.DataFrame(columns=["date", "item", "qty"]).astype({"date":"datetime64[ns]"})
    else:
        out = pd.DataFrame({
...
def build_po_excel(po_df: pd.DataFrame, order_dt: datetime.date) -> bytes:
    ...
    return output.getvalue()\n\n\ndef build_driver_excel(po_df: pd.DataFrame, order_dt: datetime.date) -> bytes:\n    """Return an .xlsx Driver Sheet (packs only)."""\n    # Keep only the essentials for the driver\n    drv = po_df[["product", "pack_size", "packs_to_order"]].copy()\n    drv = drv.rename(columns={"product": "Product", "pack_size": "Pack Size", "packs_to_order": "Packs"})\n\n    output = BytesIO()\n    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:\n        drv.to_excel(writer, sheet_name="Driver", index=False, startrow=2)\n        wb = writer.book\n        ws = writer.sheets["Driver"]\n        title = wb.add_format({"bold": True})\n        ws.write(0, 0, f"Pan Pepín — Driver Sheet (Packs Only) — {order_dt}", title)\n        for col_idx, col_name in enumerate(drv.columns):\n            width = max(12, len(str(col_name)) + 2)\n            ws.set_column(col(po_df: pd.DataFrame, order_dt: datetime.date) -> bytes:
...
st.sidebar.subheader("Starting Inventory (packs)")
start_hamb_packs   = st.sidebar.number_input("Hamburger (packs)",   min_value=0.0, value=0.0, step=0.001, format="%.3f")
start_hotdog_packs = st.sidebar.number_input("Hot Dog Buns (packs)", min_value=0.0, value=0.0, step=0.001, format="%.3f")
START_PACKS = {"Hamburger": float(start_hamb_packs), "Hot Dog Buns": float(start_hotdog_packs)}
...
on_hand = {}  # computed from START_PACKS and PACK_SIZES below
...
with prod_c1:
    if st.button("Reset products", disabled=not admin_unlocked):
        st.session_state["products_df"] = pd.DataFrame(DEFAULT_PRODUCTS)
        with c3:
    if st.button("⬇️ Download Driver Sheet (Packs)"):
        try:
            xls = build_driver_excel(po_to_export, order_date)
            st.download_button(
                label="Save DriverSheet.xlsx",
                data=xls,
                file_name=f"PanPepin_Driver_{order_date}.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            )
        except Exception as e:
            st.error(f"Driver"Products reset to defaults.")
with prod_c2:
...
# Convert beginning inventory from PACKS (3 decimals) to UNITS
on_hand = {}
for _p in VALID_PRODUCTS:
    packs_val = float(START_PACKS.get(_p, 0.0))
    pack_size_val = int(PACK_SIZES.get(_p, 1))
    on_hand[_p] = int(round(packs_val * pack_size_val))
...
with map_btn_c2:
    ...
    if mapping_csv is not None:
        try:
            imp_raw = flexible_read_table(mapping_csv)
            # Drop completely empty columns
            imp_raw = imp_raw.dropna(axis=1, how="all")
            # Standardize names (but keep originals for display if needed)
            imp = imp_raw.copy()
            imp.columns = [str(c).strip() for c in imp.columns]

            # Try to auto-detect mapping columns
            hints = _suggest_map_import(pd.DataFrame(columns=[c.lower() for c in imp.columns]))
            ...
            # If we couldn't guess, let the user choose
            if not all(hints.values()):
                st.info("Choose which columns correspond to **Item**, **Bun Kind (Product)**, and **Buns per Unit**.")
            item_col = st.selectbox("Item column", options, index=_idx(options, hints.get("item") or options[0]), key="mapimp_item")
            prod_col = st.selectbox("Bun kind / Product column", options, index=_idx(options, hints.get("product") or options[0]), key="mapimp_product")
            bpu_col = st.selectbox("Buns per unit column", options, index=_idx(options, hints.get("buns_per_unit") or options[0]), key="mapimp_bpu")

            imp2 = imp[[item_col, prod_col, bpu_col]].rename(columns={
                item_col: "item",
                prod_col: "product",
                bpu_col: "buns_per_unit",
            }).copy()

            # Normalize values
            imp2["item"] = imp2["item"].astype(str).str.strip()
            imp2["buns_per_unit"] = pd.to_numeric(imp2["buns_per_unit"], errors="coerce").fillna(0).astype(int)
            # Map product strings to canonical VALID_PRODUCTS
            imp2["product"] = imp2["product"].apply(lambda v: _normalize_product_value(v, VALID_PRODUCTS))
            unknown = sorted(set(imp2.loc[imp2["product"].isna(), "product"].astype(str)))
            if imp2["product"].isna().any():
                st.error("Some product values couldn't be recognized. Allowed: " + ", ".join(VALID_PRODUCTS))
                st.write("Unknown values (first 10):", unknown[:10])
            else:
                st.session_state["mapping_df"] = imp2
...
for p in all_products:
    ...
    po_rows.append({
        ...
        "on_hand_packs": round(float(START_PACKS.get(p, 0.0)), 3),
...
with col2:
    ...
    editable_po = st.data_editor(
        ...
        column_config={
            ...
            "on_hand_packs": st.column_config.NumberColumn("On Hand (packs)", disabled=True, format="%.3f"),
