...
# =========================
# Starting Inventory (packs with 3 decimals)
# =========================
st.subheader("Starting Inventory (packs)")

# Helper: enforce 3-decimal formatting for packs
fmt_packs = "%.3f"
...
for i, item in enumerate(VALID_PRODUCTS):
    with cols[i]:
        beg_inv_packs[item] = st.number_input(
            f"{item} (packs)",
            min_value=0.0,
            value=0.000,
            step=0.001,
            format=fmt_packs,
            key=f"beg_{item}_packs"
        )

# Convert packs → units **only for calculations**
beg_inv_units = {item: round(beg_inv_packs[item] * PACK_SIZES[item], 3) for item in VALID_PRODUCTS}

# Display in PACKS by default; allow optional units
show_units = st.checkbox("Show beginning inventory in units too (optional)", value=False)

rows = []
for item in VALID_PRODUCTS:
    row = {
        "Item": item,
        "Pack Size": PACK_SIZES[item],
        "Beginning Inventory (packs)": float(f"{beg_inv_packs[item]:.3f}"),
    }
    if show_units:
        row["Beginning Inventory (units)"] = beg_inv_units[item]
    rows.append(row)
