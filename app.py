...
def load_sales(uploaded_file: BytesIO) -> pd.DataFrame:
    ...
    if mapping.get("date") is None or mapping.get("date") not in df.columns:
        st.info("No date column detected. Assign a single date to all rows (e.g., the report end date).")
        chosen_date = st.date_input("Date for all rows", value=datetime.today().date(), key="fallback_single_date")
        out = pd.DataFrame({
