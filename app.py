# Retry file creation using triple-single-quoted strings to avoid parser confusion
...
TRAY_SIZE_PACKS = 8   # packs per full tray
HALF_TRAY_PACKS = TRAY_SIZE_PACKS // 2  # 4
...
# ThriveMetrics → buns mapping (1 item = 1 bun unit). (Inputs are lowercased on parse.)
ITEM_TO_BUN = {
...
def _round_up_to_half_tray(packs: float) -> int:
    """Round up to the nearest half-tray (4) multiple, minimum 0."""
    if packs <= 0:
        return 0
...
# =========================
# PDF parsing (robust + manual mapping fallback)
# =========================
...
def _extract_report_dates_from_text(text: str):
    """
    Try to extract a single date or date range from the report preamble (e.g., header 'from ... to ...').
    Returns (date_from, date_to) as date objects or (None, None).
    """
    m = re.search(r"from\s+([A-Za-z]{3,9}\s+\d{1,2},\s*\d{4}).*?to\s+([A-Za-z]{3,9}\s+\d{1,2},\s*\d{4})",
                  text, flags=re.IGNORECASE | re.DOTALL)
    if m:
        d1 = pd.to_datetime(m.group(1), errors="coerce")
        d2 = pd.to_datetime(m.group(2), errors="coerce")
        return (None if pd.isna(d1) else d1.date(),
                None if pd.isna(d2) else d2.date())
...
def _read_thrivemetrics_pdf_to_rows(file) -> Tuple[pd.DataFrame, str]:
    """Return a raw table-like DataFrame from a ThriveMetrics PDF and the full text.
    Strategy:
    1) Use pdfplumber to extract tables from each page; concatenate.
    2) If no tables are found, fallback to text parsing with a heuristic that
       looks for lines like: "<product name> .... <qty>".
    """
    try:
    ...
    # Read bytes from Streamlit uploader or file path
    try:
        file.seek(0)
        raw_bytes = file.read()
    except Exception:
    ...
    with pdfplumber.open(BytesIO(raw_bytes)) as pdf:
        for page in pdf.pages:
            ...
            # Try extracting tables with tolerances
            ...
            for tbl in extracted:
                if not tbl or len(tbl) < 2:
                    continue
                # First row is header; keep only non-empty cols
    ...
    qty = []
    for ln in lines:
        # Pattern: name ..... number (allow commas)
...
def parse_thrivemetrics_pdf(file) -> pd.DataFrame:
    """
    Normalize a ThriveMetrics PDF to columns: date, item, qty.
    - Accepts PDFs that show a Product/Sold table (no per-row date).
    - Gets the report date (or range) from the PDF text and assigns the 'to' date
      or single found date to every row.
    """
    ...
    if not all([item_col, qty_col]):
        # attempt a simple two-column interpretation
        if raw_df.shape[1] >= 2:
...
# =========================
# Weekly aggregation from items → bun units
# =========================
...
# ——— Pre-init for optional PDF rows ———
items_df: Optional[pd.DataFrame] = None

# =========================
# Data Source: ThriveMetrics PDF or Manual
# =========================
st.subheader("Sales Data Source")
mode = st.radio("Choose input method", ["Upload ThriveMetrics PDF (recommended)", "Manual weekly entry"], index=0)
...
if mode.startswith("Upload ThriveMetrics PDF"):
    ...
    try:
        ...
        st.success("PDF parsed. Weekly bun unit totals generated.")
    ...
    with colB:
        st.download_button(
            "⬇️ Download Weekly Sales History CSV",
...
# =========================
# QA / Dev Check (compact)
# =========================
with st.expander("🧪 Quick QA Check (dev only)", expanded=True):
    # Week window from last row in 'hist' (Sun–Sat)
    if not hist.empty:
    ...
    # If a PDF was uploaded, show raw items detected and their bun mapping
    if items_df is not None and not items_df.empty:
        c1, c2 = st.columns(2)
        with c1:
            st.caption("Items detected in PDF (top 8)")
    ...
    # Average daily packs that will feed the simulation
    ...
    # Rounding preview
    try:
        _proj_at_delivery, _ = simulate_consumption_until_delivery(
            on_hand_packs, avg_daily_packs, datetime.now(), next_order_and_delivery(now)[1]
        )
        _calc_need, _raw_reco = recommend_po(
            on_hand_packs, avg_daily_packs, PAR_DAYS_DEFAULT, datetime.now(), next_order_and_delivery(now)[1], enforce_trays=False
        )
        _rounded_reco = recommend_po(
            on_hand_packs, avg_daily_packs, PAR_DAYS_DEFAULT, datetime.now(), next_order_and_delivery(now)[1], enforce_trays=True
        )[1]
        ...
        for p in VALID_PRODUCTS:
            prev_rows.append(
                {
                    "Product": p,
                    "Projected @ Delivery (packs)": round(_proj_at_delivery[p], 3),
                    "Avg Daily (packs)": round(avg_daily_packs[p], 3),
                    "Calc Need (packs)": round(_calc_need[p], 3),
                    "Recommended (free)": int(_raw_reco[p]),
                    f"Recommended (½-tray {HALF_TRAY_PACKS})": int(_rounded_reco[p]),
                }
            )
...
po_calc_need, po_reco = recommend_po(
    on_hand_packs, avg_daily_packs, par_days, datetime.now(), delivery_dt, enforce_trays
)
    ...
    if prev_summary is None:
        ...
        show_prev = pd.DataFrame(
            [
                {
                    "Product": "Hamburger",
                    "Units": prev_summary["Hamburger Units"],
                    "Packs (ceil)": np.ceil(
                        prev_summary["Hamburger Units"] / PACK_SIZES["Hamburger"]
                    ),
                },
                {
                    "Product": "Hot Dog Buns",
                    "Units": prev_summary["Hot Dog Buns Units"],
                    "Packs (ceil)": np.ceil(
                        prev_summary["Hot Dog Buns Units"] / PACK_SIZES["Hot Dog Buns"]
                    ),
                },
            ]
        )
...
st.write(
    f"**Order Date:** {order_dt.date()}  •  **Delivery:** {delivery_dt}  •  **PAR:** {par_days} days"
)
po_rows = []
for p in VALID_PRODUCTS:
...
# 🔧 Allow editing of the PO (packs), respecting tray rounding if enabled later
st.info(
    "You can edit 'Recommended PO (packs)' below. If 'Enforce multiples of 4 packs' is ON, downloads/history will re-round to the nearest 4."
)
po_df_editable = st.data_editor(
    po_df,
    use_container_width=True,
    column_config={
        "Recommended PO (packs)": st.column_config.NumberColumn(min_value=0, step=1),
    },
    key="po_editor",
)

# Apply tray rounding for output/export if enforced
po_df_final = po_df_editable.copy()
if enforce_trays:
    po_df_final["Recommended PO (packs)"] = po_df_final["Recommended PO (packs)"].apply(
        _round_up_to_half_tray
    )
po_df_final["Recommended PO (units)"] = po_df_final.apply(
    lambda r: int(r["Recommended PO (packs)"] * r["Pack Size (units/pack)"]), axis=1
)
...
def build_po_excel(po_df: pd.DataFrame, order_dt: datetime, delivery_dt: datetime, par_days: int):
    ...
    with pd.ExcelWriter(bio, engine=engine) as w:
        pd.DataFrame(
            {"Field": ["Order Date", "Delivery", "PAR Days"], "Value": [order_dt.date(), delivery_dt, par_days]}
...
st.markdown("---")
st.caption(
    "Upload your ThriveMetrics **PDF** and we'll convert menu items to bun demand automatically: "
...
# System deps for Pillow/ReportLab (JPEG, zlib, freetype) and basic build tools
...
# Prevent Python from writing .pyc files & buffer logs less
ENV PYTHONDONTWRITEBYTECODE=1 \
    PYTHONUNBUFFERED=1

# Make and switch to app directory
WORKDIR /app
...
# Streamlit needs to listen on 0.0.0.0 for Docker
EXPOSE 8501

# Optional Streamlit config via env (can also be CLI flags)
...
(base / ".dockerignore").write_text(dockerignore, encoding="utf-8")

# Create zip
zip_path = base / "pepin_par_app_bundle.zip"
with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zf:
    zf.write(base / "app.py", arcname="app.py")
    zf.write(base / "requirements.txt", arcname="requirements.txt")
    zf.write(base / "Dockerfile", arcname="Dockerfile")
    zf.write(base / ".dockerignore", arcname=".dockerignore")
