...
def parse_thrivemetrics_csv(file) -> pd.DataFrame:
    ...
    Flexible header detection to handle exports with slight variations."""
    raw = pd.read_csv(file)
    df = normalize_columns(raw)
    item_col = pick_column(df, ["item","menu item","product","name","description","modifier name"]) or "item"
    qty_col  = pick_column(df, ["qty","quantity","units","sold","count","modifier sold"]) or "qty"
    date_col = pick_column(df, ["date","business date","order date","created","created at"]) or "date"
    if item_col not in df.columns or qty_col not in df.columns or date_col not in df.columns:
...
def buns_weekly_from_thrives(df_items: pd.DataFrame) -> pd.DataFrame:
    ...
    days_to_sun = (weekday + 1) % 7  # how many days to subtract to reach Sunday
    ...
    # Pivot to hamburger/hot dog UNITS per week
...
if mode.startswith("Upload"):
    ...
    # Manual fallback UI identical to prior versions
...
if not prev_row.empty:
    ws = pd.to_datetime(prev_row.iloc[0]["WeekStart"]).date(); we = pd.to_datetime(prev_row.iloc[0]["WeekEnd"]).date()
...
ww = delivery_dt.weekday(); week_tue = (delivery_dt + timedelta(days=(1-ww))).date(); week_wed = (delivery_dt + timedelta(days=(2-ww))).date(); week_thu = (delivery_dt + timedelta(days=(3-ww))).date()
risk_rows = []
for p in VALID_PRODUCTS:
    ...
    for dname, dval in [("Tuesday", week_tue),("Wednesday", week_wed),("Thursday", week_thu)]:
        ...
        if not row.empty and float(row[f"{p} EoD Stock (packs)"].values[0]) < 0: risk[dname] = True
...
def build_po_pdf(po_df: pd.DataFrame, order_dt: datetime, delivery_dt: datetime, par_days: int) -> bytes:
    ...
    elems += [Paragraph("Pan Pepín – Purchase Order", styles["Title"]), Paragraph(f"Order Date: {order_dt.date()}  •  Delivery: {delivery_dt}  •  PAR: {par_days} days", styles["Normal"]), Spacer(1,12)]
    ...
    for _,r in po_df.iterrows():
        ...
        data.append(row)
    t=Table(data, hAlign="LEFT"); t.setStyle(TableStyle([["BACKGROUND",(0,0),(-1,0),colors.HexColor("#f0f0f0")],["GRID",(0,0),(-1,-1),0.5,colors.grey],["ALIGN",(1,1),(-1,-1),"RIGHT"),("FONTNAME",(0,0),(-1,0),"Helvetica-Bold")]))
    elems.append(t); doc.build(elems); return bio.getvalue()
...
if st.button("✅ Create Purchase Order and Save to History"):
    # flatten to dict for history (packs only)
