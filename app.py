# Recreate patched app.py and produce a full bundle ZIP
...
def _pick_writable_base() -> Path:
    ...
    candidates += [
        Path("/tmp/pepin"),
        Path(home := Path.home()) / ".cache" / "pepin",
        Path.cwd() / ".pepin",
...
for p in VALID_PRODUCTS:
    ...
    po_rows.append({"Product": p, "Projected On-Hand at Delivery (packs)": round(proj_at_delivery[p], 3), "Avg Daily (packs)": round(avg_daily_packs[p], 3), "Target Stock (packs)": round(par_days * avg_daily_packs[p], 3), "Calc Need (packs)": po_calc_need[p], "Recommended PO (packs)": int(packs_reco), "Pack Size (units/pack)": PACK_SIZES[p], "Recommended PO (units)": int(packs_reco * PACK_SIZES[p])})
...
po_df_editable = st.data_editor(po_df, use_container_width=True, column_config={"Recommended PO (packs)": st.column_config.NumberColumn(min_value=0, step=1)}, key="po_editor")
...
def build_po_pdf(po_df: pd.DataFrame, order_dt: datetime, delivery_dt: datetime, par_days: int) -> bytes:
    ...
    t.setStyle(TableStyle([("BACKGROUND", (0, 0), (-1, 0), colors.HexColor("#f0f0f0")), ("GRID", (0, 0), (-1, -1), 0.5, colors.grey), ("ALIGN", (1, 1), (-1, -1), "RIGHT"), ("FONTNAME", (0, 0), (-1, 0), "Helvetica-Bold")]))
    elems.append(t)
    doc.build(elems)
    return bio.getvalue()
...
if st.button("✅ Create Purchase Order and Save to History"):
    ...
    for p, qty in po_dict.items():
        rows.append({"OrderDate": order_dt.date(), "DeliveryDateTime": delivery_dt, "Product": p, "PacksOrdered": qty, "PackSize": PACK_SIZES[p], "PAR_Days": par_days, "AvgDailyPacks": round(avg_daily_packs.get(p, 0.0), 3), "TrayRule": "half-tray multiples" if enforce_trays else "free"})
...
st.markdown("---")
st.caption("Upload your ThriveMetrics **PDF** and we'll convert menu items to bun demand automatically: Shack Burger Single → Hamburger bun, Shack Double Burger → Hamburger bun, Hot Dog Quarter Pound → Hot Dog bun.\\nAverage daily packs = (weekly bun units ÷ pack size) ÷ 7. PAR is applied to stock level right after Friday 5:00 AM delivery.")
"""

# Write patched app
patched_path = base / "app_patched.py"
patched_path.write_text(app_code, encoding="utf-8")

# Create bundle zip with patched app + existing deployment files if present
bundle = base / "pepin_par_app_bundle_patched.zip"
with zipfile.ZipFile(bundle, "w", zipfile.ZIP_DEFLATED) as zf:
    zf.write(patched_path, arcname="app.py")
    for fname in ("requirements.txt", "Dockerfile", ".dockerignore"):
        p = base / fname
        if p.exists():
