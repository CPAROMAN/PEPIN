...
# =========================
# ThriveMetrics CSV parsing (robust: synonyms + header autodetect + heuristic)
# =========================
def parse_thrivemetrics_csv(file) -> pd.DataFrame:
    """
    Parse ThriveMetrics-like CSVs into columns: date, item, qty.
    - Robust delimiter/BOM handling
    - Tries many header synonyms
    - Auto-detects header row if the file starts with title/junk lines
    - Heuristic fallback if names still don't match
    """
    ...
    # --- helpers ---
    def _read(delim=None, header="infer", skiprows=None, nrows=None):
        kw = dict(engine="python", encoding="utf-8-sig", skipinitialspace=True, on_bad_lines="skip")
        if skiprows is not None: kw["skiprows"] = skiprows
        if header is not None: kw["header"] = header
        if nrows is not None: kw["nrows"] = nrows
        if delim is None:
            return pd.read_csv(file, sep=None, **kw)   # sniff
        else:
            try: file.seek(0)
            except: pass
            return pd.read_csv(file, delimiter=delim, **kw)

    # 1) Read once (sniff delimiter)
    try:
        raw = _read()
    except Exception:
        raw = _read(",")

    # 1a) If it came in as single wide column, try common delimiters
    ...
    # 2) Try to auto-detect the real header row if the first rows are junk
    ...
    if looks_like_bad_header or any(str(c).startswith("Unnamed") for c in raw.columns):
        try:
            preview = _read(header=None, nrows=20)
            ...
            raw = _read(header=best_i, skiprows=None)
        except Exception:
            pass
    ...
    def pick_col(syns):
        for c in df.columns:
            if c in syns:
                return c
        # also match variants where syn appears as substring
        for c in df.columns:
            for s in syns:
                if s in c:
                    return c
    ...
    date_col = pick_col(DATE_SYNS)
    item_col = pick_col(ITEM_SYNS)
    qty_col  = pick_col(QTY_SYNS)

    # 4) Heuristic fallback if any missing: detect by data
    def detect_date():
        scores = {}
        for c in df.columns:
            try:
                s = pd.to_datetime(df[c], errors="coerce")
                scores[c] = s.notna().mean()
            except Exception:
                scores[c] = -1
        c = max(scores, key=scores.get)
        return c if scores[c] >= 0.4 else None

    def detect_qty(exclude):
        scores = {}
        for c in df.columns:
            if c == exclude: 
                scores[c] = -1; continue
    ...
    def detect_item(exclude):
        ...
        c = max(scores, key=scores.get)
        if scores[c] < 0.2:
            scores = {k: (textiness(df[k]) if k not in exclude else -1) for k in df.columns}
            c = max(scores, key=scores.get)
            return c if scores[c] >= 0.2 else None
        return c

    if date_col is None: date_col = detect_date()
    if qty_col  is None: qty_col  = detect_qty(exclude=date_col)
    if item_col is None: item_col = detect_item(exclude={date_col, qty_col})

    if not all([date_col, item_col, qty_col]):
        raise ValueError("Could not find columns for item, quantity, and date in this CSV.")

    # Build normalized output
    out = pd.DataFrame({
        "date": pd.to_datetime(df[date_col], errors="coerce").dt.date,
        "item": df[item_col].astype(str).str.strip().str.lower(),
...
if mode.startswith("Upload"):
    ...
    try:
        ...
        st.caption("Detected normalized columns (first 5 rows):")
        st.dataframe(items_df.head(), use_container_width=True)
