...
# ThriveMetrics → buns mapping (1 item = 1 bun unit). (CSV is lowercased on parse.)
ITEM_TO_BUN = {
...
def _extract_report_dates_from_text(text: str):
    """
    Try to extract a single date or date range from the report preamble (e.g., A2 line 'from ... to ...').
...
def _read_csv_any(file):
    ...
    return df, text  # return raw text for date extraction
...
def parse_thrivemetrics_csv_or_manual(file) -> pd.DataFrame:
    """
    Return normalized df with columns: date, item, qty.
    - Handles reports where the table has Product/Sold but NO date column.
    - If no date column is found, we extract the date/range from the preamble (A2)
      and assign the 'to' date (or the single found date) to all rows.
    - Still shows manual mapping UI if needed.
    """
    df, raw_text = _read_csv_any(file)
    if df.empty:
        raise ValueError("CSV appears to be empty.")

    # Prefer common labels even if the CSV doesn't include "date" as a column.
    date_col = _pick_col(df, DATE_SYNS)
    item_col = _pick_col(df, ITEM_SYNS.union({"product"}))  # include "product"
    qty_col  = _pick_col(df, QTY_SYNS.union({"sold"}))      # include "sold"
    ...
    if not all([item_col, qty_col]):
        st.warning("Could not automatically detect Item/Quantity. Please map them below:")
        st.dataframe(df.head(20), use_container_width=True)
        cols = list(df.columns)
        c1,c2,c3 = st.columns([1,1,1])
        with c1: item_col = st.selectbox("Item column", options=cols, index=0 if cols else None)
        with c2: qty_col  = st.selectbox("Quantity column", options=cols, index=1 if len(cols)>1 else 0)
        with c3: date_col = st.selectbox("Date column (optional)", options=["<none>"] + cols, index=0)
    ...
    if date_col:
        ...
        d_from, d_to = _extract_report_dates_from_text(raw_text)
        chosen = d_to or d_from or datetime.now().date()
        dates = pd.Series([chosen] * len(df), dtype="object")

    out = pd.DataFrame({"date": dates, "item": items, "qty": qty}).dropna(subset=["date"])
...
if not prev_row.empty:
    ...
    we = pd.to_datetime(prev_row.iloc[0]["WeekEnd"]).date()  # fixed scalar .date()
