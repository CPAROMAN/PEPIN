...
# =========================
# Credential getters (UI overrides > env vars)
# =========================
def _get_twilio_sid():
    return st.session_state.get("TWILIO_SID_OVERRIDE") or os.getenv("TWILIO_SID")

def _get_twilio_auth():
    return st.session_state.get("TWILIO_AUTH_OVERRIDE") or os.getenv("TWILIO_AUTH")

def _get_sendgrid_api_key():
    return st.session_state.get("SENDGRID_API_KEY_OVERRIDE") or os.getenv("SENDGRID_API_KEY")
...
with st.sidebar:
    ...
    with st.expander("🔐 Messaging credentials (optional)", expanded=False):
        tw_sid_input = st.text_input("Twilio SID (overrides env if set)", value="", type="password")
        tw_auth_input = st.text_input("Twilio Auth Token (overrides env if set)", value="", type="password")
        sg_api_input = st.text_input("SendGrid API Key (overrides env if set)", value="", type="password")
        wa_from_input = st.text_input("Twilio WhatsApp sender", value=st.session_state.get("WA_SANDBOX_FROM", "whatsapp:+14155238886"))
        wa_to_hint = st.text_input("Default WhatsApp recipient (optional)", value=st.session_state.get("WA_DEFAULT_TO", ""))

        if tw_sid_input:
            st.session_state["TWILIO_SID_OVERRIDE"] = tw_sid_input
        if tw_auth_input:
            st.session_state["TWILIO_AUTH_OVERRIDE"] = tw_auth_input
        if sg_api_input:
            st.session_state["SENDGRID_API_KEY_OVERRIDE"] = sg_api_input
        st.session_state["WA_SANDBOX_FROM"] = wa_from_input
        if wa_to_hint:
            st.session_state["WA_DEFAULT_TO"] = wa_to_hint
...
col_sms1, col_sms2, col_sms3 = st.columns([1,1,1])
with col_sms1:
...
with col_sms3:
    if st.button("🧪 Test message text (show below)"):
        st.code(sms_text)

with st.expander("✉️ Send via SMS (Twilio)", expanded=False):
    st.caption("Provide a Twilio 'from' number and recipient. Uses inline credentials if set, else env vars.")
    ...
    if enable_twilio:
        ...
        with c1:
            if st.button("📲 Send PO SMS via Twilio"):
                try:
                    ...
                    sid = _get_twilio_sid(); auth = _get_twilio_auth()
                    if not sid or not auth:
                        st.error("Missing TWILIO_SID / TWILIO_AUTH (set in credentials expander or env).")
        ...
        with c2:
            if st.button("🧪 Send test SMS (Hello from Pan Pepín)"):
                try:
                    from twilio.rest import Client  # type: ignore
                    sid = _get_twilio_sid(); auth = _get_twilio_auth()
                    if not sid or not auth:
                        ...
                        client = Client(sid, auth)
                        msg = client.messages.create(body="Test: Hello from Pan Pepín", from_=from_number, to=to_number)
                        st.success(f"Test SMS sent! SID: {msg.sid}")
                except Exception as e:
                    st.error(f"Failed to send test SMS: {e}")

with st.expander("💬 Send via WhatsApp (Twilio)", expanded=False):
    st.caption("Use your Twilio WhatsApp sandbox or approved WhatsApp numbers. Numbers must start with 'whatsapp:+'.")
    enable_wa = st.checkbox("Enable WhatsApp send", value=False, key="wa_enable")
    if enable_wa:
        wa_to = st.text_input("Recipient WhatsApp (whatsapp:+E.164)", value=st.session_state.get("WA_DEFAULT_TO", ""), key="wa_to")
        wa_from = st.text_input("Your Twilio WhatsApp sender (whatsapp:+E.164)", value=st.session_state.get("WA_SANDBOX_FROM", "whatsapp:+14155238886"), key="wa_from")
        c1, c2 = st.columns(2)
        with c1:
        ...
        with c2:
            if st.button("🧪 Send test WhatsApp (Hello from Pan Pepín)"):
                try:
                    from twilio.rest import Client  # type: ignore
                    sid = _get_twilio_sid(); auth = _get_twilio_auth()
                    if not sid or not auth:
                        st.error("Missing TWILIO_SID / TWILIO_AUTH (set in credentials expander or env).")
                    elif not wa_to or not wa_from:
                        st.error("Please fill both WhatsApp numbers (from/to).")
                    elif not wa_to.startswith("whatsapp:+") or not wa_from.startswith("whatsapp:+"):
                        st.error("Numbers must start with 'whatsapp:+'.")
                    else:
                        client = Client(sid, auth)
                        msg = client.messages.create(body="Test: Hello from Pan Pepín", from_=wa_from, to=wa_to)
                        st.success(f"Test WhatsApp sent! SID: {msg.sid}")
                except Exception as e:
                    st.error(f"Failed to send test WhatsApp: {e}")

with st.expander("📧 Send via Email (SendGrid)", expanded=False):
    st.caption("Set SendGrid key in credentials expander or env. Choose plain-text or HTML (table).")
    ...
    if enable_email:
        ...
        with c1:
            if st.button("✈️ Send PO Email (SendGrid)"):
                try:
                    ...
                    if not api_key:
                        st.error("Missing SENDGRID_API_KEY (set in credentials expander or env).")
        ...
        with c2:
            if st.button("🧪 Send test Email (plain text)"):
                try:
                    import requests
                    api_key = _get_sendgrid_api_key()
                    if not api_key:
                        ...
                        data = {
                            "personalizations": [{"to": [{"email": to_email}]}],
                            "from": {"email": from_email},
                            "subject": "Test: Hello from Pan Pepín",
                            "content": [{"type": "text/plain", "value": "Test email from Pan Pepín"}],
                        }
                        resp = requests.post(
                            "https://api.sendgrid.com/v3/mail/send",
                            headers={"Authorization": f"Bearer {api_key}", "Content-Type": "application/json"},
                        ...
                        if resp.status_code in (200, 202):
                            st.success("Test email accepted by SendGrid.")
                        else:
                            st.error(f"SendGrid error {resp.status_code}: {resp.text}")
                except Exception as e:
...
- **Twilio**: set `TWILIO_SID`, `TWILIO_AUTH` or paste them in the credentials expander. WhatsApp numbers must be prefixed with `whatsapp:+`.
- **SendGrid**: set `SENDGRID_API_KEY` or paste it in the credentials expander. We send either plain text or a simple HTML table.
""")
