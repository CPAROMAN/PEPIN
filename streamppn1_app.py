...
def lib_info(name: str):
    """Return (installed: bool, version: str|None) for a given module name."""
...
def pip_install(pkgs):
    """Attempt to pip install a list of packages at runtime. Returns (ok, output)."""
    try:
        ...
        # Build output safely without embedded literal newlines in quotes
        out = "
...
def build_po_excel(po_df: pd.DataFrame, order_dt: datetime, delivery_dt: datetime, par_days: int):
    ...
    Returns (bytes_or_None, error_message_or_None). Uses xlsxwriter if available, else openpyxl.
    """
    # Pick an engine that's installed
    engine = None
    try:
        ...
        try:
            ...
            return None, "Excel export requires 'xlsxwriter' or 'openpyxl'. Install with: pip install xlsxwriter"
    ...
    with pd.ExcelWriter(bio, engine=engine) as writer:
        ...
        # Optional: format numeric columns to 3 decimals when engine supports it
        try:
...
with st.sidebar:
    ...
    for lib in libs:
        ok, ver = lib_info(lib)
        icon = "✅" if ok else "❌"
        ver_txt = f" v{ver}" if (ok and ver) else ""
        st.write(f"{icon} **{lib}**{ver_txt}")
        if not ok:
            missing.append(lib)
    if missing:
        st.caption("Install missing: " + " ".join([f"`pip install {m}`" for m in missing]))
        if st.button("Install ALL missing packages", key="install_all_missing"):
            ok, out = pip_install(missing)
            if out:
                st.code(out[-4000:])
            if ok:
                st.success("Installation complete. Click 'Rerun' from the app menu to reload imports.")
            else:
                st.error("Installation failed. See logs above. On Streamlit Cloud, add these to requirements.txt.")
        for m in missing:
            if st.button(f"Install {m}", key=f"install_{m}"):
                ok, out = pip_install([m])
                if out:
                    st.code(out[-4000:])
                if ok:
                    st.success(f"Installed {m}. Click 'Rerun' to reload.")
                else:
                    st.error(f"Failed to install {m}. If you're on Streamlit Cloud, add it to requirements.txt.")
    else:
        st.caption("All export libraries installed.")
