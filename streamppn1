...
def simulate_consumption_until_delivery(on_hand_packs: dict, avg_daily_packs: dict, now: datetime, delivery_dt: datetime):
...
avg_daily_packs = {}
for p in VALID_PRODUCTS:
    units_col = f"{p} Units" if p == "Hamburger" else "Hot Dog Buns Units"
    if p == "Hamburger":
        units_col = "Hamburger Units"
    else:
        units_col = "Hot Dog Buns Units"
...
# Simulate consumption from today until delivery using avg daily packs
...
# Stockout risk Tue/Wed/Thu based on projected EoD stock
risk_rows = []
for p in VALID_PRODUCTS:
    risk = {"Tuesday": False, "Wednesday": False, "Thursday": False}
    for dname, target_wd in [("Tuesday", 1), ("Wednesday", 2), ("Thursday", 3)]:
        # Find the row for that weekday this week
